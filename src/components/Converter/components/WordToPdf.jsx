...
const WordToPdf = () => {
    ...
    const handleConvert = async () => {
        ...
        try {
            ...
            setResultBlob(blob);
            addHistoryEntry(`${file.name.replace(/\.[^/.]+$/, '')}.pdf`, 'Word to PDF', 'Success');
        } catch (err) {
            console.error(err);
            alert('Error converting document to PDF.');
            addHistoryEntry(file.name, 'Word to PDF Operation', 'Failed');
        } finally {
            setProcessing(false);
        }
    };
    ...
    return (
        ...
        >
            <div className="icon-wrapper" style={{ background: 'linear-gradient(135deg, #2b6cb0 0%, #4299e1 100%)' }}>
                <Icon name='file word' size='huge' style={{ color: 'white' }} />
            </div>
            <SemanticHeader as='h2' style={{ color: 'white' }}>Word to PDF</SemanticHeader>
            ...
            <div {...getRootProps()} className={`dropzone ${isDragActive ? 'active' : ''}`}>
                ...
                {file ? <p style={{ color: 'white', fontWeight: '600' }}>{file.name}</p> : <p>Drag & drop Word document here, or click to browse</p>}
            </div>
            ...
            <div className="controls">
                <Button 
                    ...
                    loading={processing} 
                    disabled={!file}
                    style={{ width: '100%', background: 'linear-gradient(135deg, #2b6cb0 0%, #4299e1 100%)', color: 'white' }}
                >
                    Convert to PDF
                </Button>
            </div>
